            ...
            {group.items.map(it => {
              ...
              return (
                <Link key={it.to} to={it.to} className={`nav-item mx-2 ${active ? "active" : ""}`}>
                  <Icon size={16} className="text-ghost" />
...
export function Topbar() {
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 px-3 py-1 border border-lime-border rounded-sm bg-lime-dim/30">
      ...
      <div className="flex items-center gap-4 ml-auto">
        ...
        <div className="w-7 h-7 rounded-full bg-lime grid place-items-center text-void text-xs font-bold">A</div>
      </div>
    </header>
  );
}
...
export function StatCard({ label, value, color = "white", trend, hero }: { label: string; value: string; color?: "white" | "lime" | "threat"; trend?: string; hero?: boolean }) {
  ...
  return (
    <div className={`ps-card ${hero ? "ps-card-lime" : ""}`}>
      ...
      <div className={`display text-[56px] leading-none ${colorClass} ${color === "threat" ? "lime-glow" : ""}`} style={color === "threat" ? { boxShadow: "none", textShadow: "0 0 20px rgba(239,68,68,0.4)" } : undefined}>
        {value}
      </div>
