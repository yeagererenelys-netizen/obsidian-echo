  ...
  return (
    <div>
      ...
      <div className="flex gap-1 border-b border-graphite mb-6">
        {TABS.map(t => (
          <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 display text-base ${tab===t?"text-lime border-b-2 border-lime":"text-ghost hover:text-silver"}`}>{t}</button>
        ))}
      </div>
      {tab === "HTTP" && (
        ...
        <div className="ps-card">
          ...
          {[
            ...
            <div key={r.v} className="mb-3">
              ...
              <div className="h-2 bg-carbon rounded"><div className={`h-full ${r.c} rounded ${r.v==="SSL 3.0"?"lime-glow":""}`} style={{ width: `${r.pct}%` }} /></div>
            </div>
          ))}
        </div>
      )}
