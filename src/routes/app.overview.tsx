...
const talkers = [
  { ip: "192.168.1.45", name: "workstation-04", val: 92 },
  { ip: "192.168.1.10", name: "fileserver-01", val: 78 },
  { ip: "192.168.1.89", name: "laptop-eng-12", val: 64 },
  { ip: "192.168.1.234", name: "unknown", val: 51 },
  { ip: "192.168.1.42", name: "dev-box-02", val: 33 },
];
const alerts = [
  { sev: "threat", t: "12:42:18", title: "Beaconing detected", src: "192.168.1.45", dst: "167.88.162.34" },
  { sev: "warn", t: "12:41:02", title: "VPN/OpenVPN traffic", src: "192.168.1.89", dst: "UDP 1194" },
  { sev: "threat", t: "12:39:55", title: "Tor exit node connection", src: "192.168.1.234", dst: "185.220.101.45" },
  { sev: "warn", t: "12:38:11", title: "Suspicious DNS query", src: "192.168.1.42", dst: "update.verysuspicious-domain.cc" },
  { sev: "threat", t: "12:36:40", title: "Port scan detected", src: "192.168.1.200", dst: "192.168.1.105 (847 ports)" },
];
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<>
          <button className="btn btn-primary"><Plus size={14} /> Start Capture</button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard label="Devices Online" value="34" trend="↑ 1 new" />
      </div>
            ...
            {talkers.map(t => (
              <div key={t.ip}>
                <div className="flex justify-between mono text-xs mb-1">
                  ...
                  <span className="text-ghost">{t.val}MB</span>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        <div className="ps-card relative overflow-hidden">
          ...
          <div className="space-y-2">
            {alerts.map((a, i) => (
              <div key={i} className={`flex items-start gap-3 p-2 rounded border-l-2 ${
                a.sev === "threat" ? "border-threat bg-threat-dim/30" : "border-warn bg-warn-dim/20"
              }`}>
                <span className={`dot ${a.sev === "threat" ? "dot-threat" : "dot-warn"} mt-1.5`} />
        ...
        <div className="ps-card">
          <div className="flex items-center justify-between mb-4">
            ...
            <a href="/app/graph" className="text-xs text-lime hover:underline">View Full Graph →</a>
          </div>
          <svg viewBox="0 0 400 240" className="w-full h-[240px]">
            {Array.from({ length: 18 }).map((_, i) => {
              ...
              return (
                <g key={i}>
                  <line x1={200} y1={120} x2={x} y2={y} stroke="rgba(255,255,255,0.15)" strokeWidth={0.5} />
                </g>
              );
            })}
            {Array.from({ length: 18 }).map((_, i) => {
              ...
              return (
                <circle key={i} cx={x} cy={y} r={threat ? 5 : 3} fill={threat ? "#ef4444" : "#a0aec0"} />
              );
            })}
            <circle cx={200} cy={120} r={10} fill="#fff" />
            <circle cx={200} cy={120} r={14} fill="none" stroke="#a3ff12" strokeWidth={1} opacity={0.6} />
          </svg>
          <div className="mono text-xs text-ghost mt-2">34 nodes · 287 edges · 2 flagged</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="ps-card ps-card-lime">
          ...
          <a href="/app/beaconing" className="btn btn-secondary !text-[11px] !py-1 mt-3">Investigate →</a>
        </div>
        <div className="ps-card">
